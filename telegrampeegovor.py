...
def load_users_with_free_lessons():
    ...
    # Получить все значения в первом столбце (идентификаторы пользователей)
    user_ids = worksheet.col_values(1)

    # Начиная со второй строки (первая строка - заголовки), пройдитесь по каждой строке
    for user_id in user_ids[:]:
        users_with_free_lessons.add(int(user_id))


# Вызовите эту функцию в начале работы бота
load_users_with_free_lessons()
